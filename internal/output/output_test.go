...
func TestOutputFromNeutral(t *testing.T) {
	config := decisionpkg.Config{UseTags: true, UseFieldDetection: true, UseValueDetection: false, Detector: detection.NewDetector(detection.DefaultRules(), false), Preservation: redactcore.DefaultConfig()}
	builder := New(config, decisionpkg.New(config), copyinternal.NewTypeCache())
