...
func (w *Walker) BlobFromNeutral(input any) (any, error) { return w.blobFromNeutralValue(input, "", "") }
...
func (w *Walker) blobFromReflect(v reflect.Value, format, fieldName, tag string, allowDecision bool) (any, error) {
	...
	if allowDecision {
		dec, err := w.decide(fieldName, tag, valueInterface(v)); if err != nil { return nil, err }
		if dec.skip { return w.plainFromReflect(v, format) }
		if dec.remove { return omitValue, nil }
		if dec.strategyName != "" {
			strategyValue, err := w.applyStrategy(v, dec.strategyName); if err != nil { return nil, err }
			return valueInterface(strategyValue), nil
		}
	}
	switch v.Kind() {
	case reflect.Interface:
		if v.IsNil() { return nil, nil }
...
func (w *Walker) plainFromReflect(v reflect.Value, format string) (any, error) { return w.blobFromReflect(v, format, "", "", false) }

func (w *Walker) blobFromNeutralValue(input any, fieldName, tag string) (any, error) {
	dec, err := w.decide(fieldName, tag, input); if err != nil { return nil, err }
	if dec.skip { return cloneNeutral(input), nil }
	if dec.remove { return omitValue, nil }
	if dec.strategyName != "" { return redactcore.Apply(dec.strategyName, input, w.cfg.Preservation) }
	switch value := input.(type) {
	case map[string]any:
		out := make(map[string]any, len(value))
		for key, item := range value {
			resolved, err := w.blobFromNeutralValue(item, key, ""); if err != nil { return nil, err }
			if resolved == omitValue { continue }
			out[key] = resolved
		}
		return out, nil
	case []any:
		...
		for _, item := range value {
			resolved, err := w.blobFromNeutralValue(item, "", ""); if err != nil { return nil, err }
