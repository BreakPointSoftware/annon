...
func (w *Walker) copyValue(v reflect.Value, fieldName string, tag string, allowDecision bool) (reflect.Value, error) {
	...
	case reflect.Struct:
		...
		for _, meta := range w.cache.StructFields(v.Type()) {
			...
			copied, err := w.copyValue(field, meta.DetectionName("typed"), meta.AnonymiseTag, true)
			if err != nil {
				return reflect.Value{}, err
			}
...
func (w *Walker) cloneValue(v reflect.Value) (reflect.Value, error) {
	if !v.IsValid() {
		return v, nil
	}
	return w.copyValue(v, "", "", false)
}

func (w *Walker) decide(fieldName, tag string, value any) (decision, error) {
	if w.cfg.UseTags {
		parsed := parseTag(tag)
		if err := w.validateTag(parsed); err != nil {
			return decision{}, err
		}
		if parsed.skip {
...
func (w *Walker) detect(fieldName string, value any) detection.Match {
	...
	if w.cfg.UseFieldDetection && w.cfg.UseValueDetection {
		return w.cfg.Detector.Detect(fieldName, value)
	}
	if w.cfg.UseFieldDetection {
		if detector, ok := w.cfg.Detector.(detection.FieldDetector); ok {
			return detector.DetectField(fieldName)
		}
		return detection.NoMatchResult()
	}
	if w.cfg.UseValueDetection {
		if detector, ok := w.cfg.Detector.(detection.ValueDetector); ok {
...
func (w *Walker) applyStrategy(v reflect.Value, strategyName string) (reflect.Value, error) {
	...
	if !ok {
		return reflect.Value{}, fmt.Errorf("unknown strategy %q", strategyName)
	}
	result, err := strategyImpl.Anonymise(valueInterface(v), strategypkg.Context{Preservation: w.cfg.Preservation})
	if err != nil {
		return reflect.Value{}, err
	}
	if result == nil {
